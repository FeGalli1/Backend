...
export const processForgotPasswordForm = async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        res.render('forgot-password', { error: 'Error al enviar el correo electrónico de restablecimiento de contraseña.' });
    }
}; 
...
export const renderResetPasswordForm = async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            return res.render('reset-password-error', { error: 'El enlace de restablecimiento de contraseña no es válido o ha expirado.' });
        }
...
export const processResetPasswordForm = async (req, res) => {
    ...
    try {
        ...
        {
            return res.render('reset-password', { error: 'La nueva contraseña debe ser diferente a la contraseña actual.',token });
        }
            user.password = createHash(newPassword);
            user.resetToken = undefined;
            user.resetTokenExpiration = undefined;
            await user.save();
            res.render('reset-password-success', { success: 'Contraseña restablecida con éxito. Puedes iniciar sesión con tu nueva contraseña.' });
    } catch (error) {
        console.error(error);
